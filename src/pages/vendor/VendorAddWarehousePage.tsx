...
function VendorAddWarehousePage() {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      formData.append('role', ROLES.VENDOR)
      ...
      if (warehouseImageFile) formData.append('warehouse_image', warehouseImageFile)
      ...
      navigate(VENDOR_ROUTES.warehouses)
    } catch (err) {
  ...
  return (
    <VendorLayout title="Add Warehouse" backTo={VENDOR_ROUTES.warehouses} backTitle="Back to Warehouses">
          ...
          <form onSubmit={handleSubmit} className="d-flex flex-column gap-3">
            <div className="row g-3">
              <div className="col-md-6">
                ...
                <input className="form-control edit-modal-input" value={name} onChange={(e) => setName(e.target.value)} required />
              </div>
              <div className="col-md-6">
                <label className="form-label fw-semibold mb-1">Code</label>
                <input className="form-control edit-modal-input" value={code} onChange={(e) => setCode(e.target.value)} maxLength={50} />
              </div>
            </div>
            ...
            <div>
              ...
              <input className="form-control edit-modal-input" value={addressLine1} onChange={(e) => setAddressLine1(e.target.value)} />
            </div>
            ...
            <div className="row g-3">
              <div className="col-md-6">
                ...
                <input type="number" step="any" className="form-control edit-modal-input" value={latitude ?? ''} onChange={(e) => setLatitude(e.target.value === '' ? null : Number(e.target.value))} />
              </div>
              <div className="col-md-6">
                <label className="form-label fw-semibold mb-1">Longitude</label>
                <input type="number" step="any" className="form-control edit-modal-input" value={longitude ?? ''} onChange={(e) => setLongitude(e.target.value === '' ? null : Number(e.target.value))} />
              </div>
            </div>
            ...
            <div>
              ...
              {imagePreviewUrl && <img className="mt-2" src={imagePreviewUrl} alt="Preview" style={{ maxWidth: 200, maxHeight: 120, objectFit: 'cover', borderRadius: 8, border: '1px solid #dee2e6' }} />}
            </div>

            <div className="row g-3">
              <div className="col-md-6">
                <label className="form-label fw-semibold mb-1">Status</label>
                <select className="form-select edit-modal-input" value={status} onChange={(e) => setStatus(Number(e.target.value) as 0 | 1)}>
              ...
              <div className="col-md-6 d-flex align-items-end">
                <div className="form-check mb-2">
                  <input className="form-check-input" type="checkbox" id="vendorIsDefaultAdd" checked={isDefault} onChange={(e) => setIsDefault(e.target.checked)} />
                  <label className="form-check-label" htmlFor="vendorIsDefaultAdd">Default warehouse</label>
                </div>
              </div>
            </div>
            ...
            <div className="d-flex justify-content-end gap-2 pt-2">
              <button type="button" className="btn btn-light border" onClick={() => navigate(VENDOR_ROUTES.warehouses)} disabled={saving}>Cancel</button>
              <button type="submit" className="btn edit-modal-save-btn px-3" disabled={saving}>{saving ? 'Saving...' : 'Save Warehouse'}</button>
            </div>
          </form>
        </div>
      </div>
...
export default VendorAddWarehousePage
